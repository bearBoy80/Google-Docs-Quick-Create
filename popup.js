...
var options,
    ...
    doc_types = ['document', 'spreadsheet', 'presentation', 'drawing', 'form','keep']
...
function enable_hotkeys() {
  document.addEventListener('keydown', (e) => {
    ...
    if ('DSPAF'.includes(key)) {
      const selector = (key === 'A' ? '[data-doc-type=drawing]' : `[data-doc-type^=${key.toLowerCase()}]`);
    ...
    if (e.key === 'ArrowUp') {
      e.preventDefault();
      const prevLink = links_wrap.querySelector('a:focus')?.parentElement.previousElementSibling?.firstElementChild;
      prevLink?.focus();
    } else if (e.key === 'ArrowDown') {
      e.preventDefault();
      const nextLink = links_wrap.querySelector('a:focus')?.parentElement.nextElementSibling?.firstElementChild;
      nextLink?.focus();
    }
  });
}
